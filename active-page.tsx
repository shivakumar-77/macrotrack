  ...
  return (
    <div style={{ background: 'var(--surface)', minHeight: '100dvh', maxWidth: 430, margin: '0 auto', paddingBottom: 24 }}>
      <style jsx>{`
        ...
        .ex-sheet-up { animation: exSheetUp 0.3s cubic-bezier(.4,0,.2,1); }
      `}</style>
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 100, background: 'var(--surface)', padding: 'calc(env(safe-area-inset-top,0px) + 10px) 16px 12px', borderBottom: '0.5px solid var(--border)', backdropFilter: 'blur(12px)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
          ...
          <button onClick={finishWorkout} disabled={saving || exercises.length === 0} className="tap-scale"
            style={{ background: '#10b981', border: 'none', borderRadius: 99, padding: '10px 20px', color: '#fff', fontWeight: 700, fontSize: 14, cursor: 'pointer', opacity: exercises.length === 0 ? 0.5 : 1, boxShadow: '0 4px 14px -4px rgba(16,185,129,0.5)' }}>
            {saving ? '…' : 'Finish'}
          </button>
        </div>
      ...
      {restActive && (
        <div className="fade-in-up" style={{ margin: '12px 16px 0', background: 'linear-gradient(135deg,#1d4ed8,#2563eb)', borderRadius: 20, padding: '16px 18px', color: '#fff', display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 5, marginBottom: 8, flexWrap: 'wrap' }}>
              {[30, 60, 90, 120].map(s => (
                <button key={s} onClick={() => startRest(s)} className="tap-scale"
            ...
            <button onClick={() => { clearInterval(restRef.current); setRestActive(false) }} className="tap-scale"
      ...
      <div style={{ padding: '14px 16px' }}>
        ...
        {exercises.map((ex, exIdx) => {
          ...
          return (
            <div key={ex.uid || exIdx} className="fade-in-up" style={{ background: 'var(--card)', borderRadius: 22, border: '1px solid var(--border)', marginBottom: 14, overflow: 'hidden', boxShadow: '0 4px 16px rgba(0,0,0,0.05)', animationDelay: `${Math.min(exIdx, 6) * 0.04}s` }}>
              {/* Exercise header */}
              <div style={{ padding: '15px 16px', background: 'var(--card2)', borderBottom: '0.5px solid var(--border)' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexShrink: 0, alignItems: 'center' }}>
                    {pr && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 3, padding: '3px 8px', background: '#fef3c7', borderRadius: 8, fontSize: 10, fontWeight: 700, color: '#d97706' }}>
                    ...
                    <button onClick={() => setShowPlateCalc(showPlateCalc === exIdx ? null : exIdx)} className="tap-scale"
                    ...
                    <button onClick={() => removeExercise(exIdx)} className="tap-scale"
                      style={{ width: 30, height: 30, borderRadius: 9, background: '#fef2f2', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ef4444' }}>
                      <IconX size={13}/>
                    </button>
                  </div>
                </div>
              ...
              {selectedEx === exIdx && (
                <div className="fade-in-up" style={{ padding: '14px 16px', background: 'var(--primary-bg)', borderBottom: '0.5px solid var(--border)' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, alignItems: 'flex-start', fontSize: 11, color: '#d97706', fontWeight: 600, background: '#fef3c7', padding: '9px 11px', borderRadius: 10 }}>
                ...
                return (
                  <div key={setIdx}>
                    <div style={{
                      ...
                      background: set.done ? 'rgba(16,185,129,0.07)' : 'transparent',
                      ...
                      <div style={{ fontSize: 11, color: 'var(--muted)', lineHeight: 1.3 }}>
                        ...
                        {isPR && set.done && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 2, fontSize: 9, color: '#f59e0b', fontWeight: 700, marginTop: 1 }}>
                      ...
                      <input type="text" inputMode="decimal" value={set.weight}
                        ...
                        style={{
                          ...
                          border: '1.5px solid ' + (set.done ? '#10b981' : 'var(--border)'),
                      ...
                      {/* Check */}
                      <button onClick={() => toggleDone(exIdx, setIdx)} className="tap-scale"
                        style={{
              ...
              <div style={{ display: 'flex', gap: 8, padding: '10px 14px 15px' }}>
                <button onClick={() => addSet(exIdx)} className="tap-scale"
                ...
                {ex.sets.length > 1 && (
                  <button onClick={() => removeSet(exIdx, ex.sets.length - 1)} className="tap-scale"
                    style={{ padding: '9px 15px', borderRadius: 11, background: '#fef2f2', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 700, color: '#ef4444', display: 'flex', alignItems: 'center', gap: 5 }}>
        ...
        {/* Add exercise */}
        <button onClick={() => setShowExPicker(true)} className="tap-scale"
      ...
      {showExPicker && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 200, display: 'flex', alignItems: 'flex-end' }}>
          <div className="ex-sheet-up" style={{ background: 'var(--surface)', width: '100%', maxWidth: 430, margin: '0 auto', borderRadius: '26px 26px 0 0', maxHeight: '86dvh', display: 'flex', flexDirection: 'column', paddingBottom: 'calc(env(safe-area-inset-bottom,0px) + 16px)' }}>
            <div style={{ padding: '14px 20px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '0.5px solid var(--border)', flexShrink: 0 }}>
              ...
              <button onClick={() => setShowExPicker(false)} className="tap-scale" style={{ background: 'var(--card2)', border: 'none', borderRadius: 10, width: 32, height: 32, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted)' }}>
                <IconX size={14}/>
              </button>
            </div>
            <div style={{ padding: '12px 16px 10px', borderBottom: '0.5px solid var(--border)', flexShrink: 0 }}>
              ...
              <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 2 }}>
                {CATS.map(c => (
                  <button key={c} onClick={() => setExCat(c)} className="tap-scale"
                ...
                return (
                  <button key={ex.id} onClick={() => addExercise(ex)} className="tap-scale"
                    ...
                    <div style={{ display: 'flex', gap: 4, flexShrink: 0, alignItems: 'center' }}>
                      {pr && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 2, padding: '2px 7px', background: '#fef3c7', borderRadius: 7, fontSize: 9, fontWeight: 700, color: '#d97706' }}>
