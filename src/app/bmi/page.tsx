    ...
    <div style={{ background: 'var(--surface)', minHeight: '100dvh', maxWidth: 430, margin: '0 auto', paddingBottom: 40 }}>
      ...
      <div style={{ padding: '0 20px' }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            {[{ id: 'male', icon: '♂', label: 'Male' }, { id: 'female', icon: '♀', label: 'Female' }].map(g => (
              <button key={g.id} onClick={() => setGender(g.id)}
                style={{ flex: 1, padding: '20px', borderRadius: 20, border: '2px solid ' + (gender === g.id ? 'var(--primary)' : 'var(--border)'), background: gender === g.id ? 'var(--primary-bg)' : 'var(--card)', cursor: 'pointer', textAlign: 'center', transition: 'all 0.15s' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 20 }}>
          {[{ label: 'Height', unit: 'cm', val: height, set: setHeight }, { label: 'Weight', unit: 'kg', val: weight, set: setWeight }, { label: 'Age', unit: 'yrs', val: age, set: setAge }].map(f => (
