...
export default function Dashboard() {
  ...
  const totals = logs.reduce((a, l) => ({
    cal: a.cal + l.cal, protein: a.protein + l.protein,
    carb: a.carb + l.carb, fat: a.fat + l.fat, fiber: a.fiber + l.fiber
    ...
    <div className="page">
      <div style={{ paddingTop: 24, paddingBottom: 4, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{
          ...
          background: profile?.goal === 'lose' ? '#d1fae5' : profile?.goal === 'gain' ? '#dbeafe' : '#fef3c7',
          color: profile?.goal === 'lose' ? '#059669' : profile?.goal === 'gain' ? '#2563eb' : '#d97706'
        }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginTop: 20 }}>
        <MacroRing
          cal={totals.cal} calTarget={g.cal_target}
          protein={totals.protein} proteinTarget={g.protein_target}
          carb={totals.carb} carbTarget={g.carb_target}
          fat={totals.fat} fatTarget={g.fat_target}
          remaining={g.cal_target - totals.cal}
        />
      </div>
        ...
        {[
          ...
          <div key={m.label} className="card" style={{ padding: '14px 16px' }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700 }}>
              {Math.round(m.val)}<span style={{ fontSize: 13, color: 'var(--muted)', fontWeight: 400 }}>/{m.target}{m.unit}</span>
            </div>
      ...
      <button className="btn btn-primary pulse-primary"
        style={{ width: '100%', marginTop: 16, fontSize: 16, fontWeight: 700, padding: '16px' }}
