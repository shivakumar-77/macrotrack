...
function ActiveWorkoutContent() {
  ...
  return (
    <div style={{ background: 'var(--surface)', minHeight: '100dvh', maxWidth: 430, margin: '0 auto', paddingBottom: '20px' }}>
      ...
      {restTimer && (
        <div style={{ margin: '16px 20px 0', background: 'linear-gradient(135deg, #3b82f6 0%, #1e40af 100%)', borderRadius: 16, padding: '20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', color: '#fff', boxShadow: '0 8px 24px rgba(59, 130, 246, 0.3)' }}>
          <div>
      ...
      <div style={{ padding: '20px' }}>
              ...
              <div style={{ padding: '16px', borderBottom: expandedExercise === exIdx ? '1px solid var(--border)' : 'none', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, background: expandedExercise === exIdx ? 'var(--primary-bg)' : 'transparent', cursor: 'pointer' }} onClick={() => setExpandedExercise(expandedExercise === exIdx ? null : exIdx)}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--muted)', display: 'flex', gap: 8 }}>
                    <span>{ex.emoji} {ex.category}</span><span>•</span><span>{ex.equipment}</span><span>•</span><span>{ex.muscle}</span>
                  </div>
                </div>
        ...
        <button onClick={() => setShowExercisePicker(true)}
          style={{ width: '100%', padding: '16px', borderRadius: 14, background: 'linear-gradient(135deg, #6366f1 0%, #4f46e5 100%)', border: 'none', cursor: 'pointer', fontSize: 15, fontWeight: 800, color: '#fff', marginTop: 16, marginBottom: 16, transition: 'all 0.3s', boxShadow: '0 4px 12px rgba(99,102,241,0.3)' }}>
          + Add Exercise
        </button>
                ...
                <button key={ex.id} onClick={() => { addExercise(ex); setShowExercisePicker(false) }}
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 3, display: 'flex', gap: 6 }}>
                      <span>{ex.category}</span><span>•</span><span>{ex.equipment}</span><span>•</span><span>{ex.muscle}</span>
                    </div>
                  </div>
      ...
      {showMetricsModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)', padding: '20px' }}>
          <div style={{ background: 'var(--card)', borderRadius: 20, padding: '24px', maxWidth: '100%', width: '100%', boxShadow: '0 16px 48px rgba(0,0,0,0.2)' }}>
