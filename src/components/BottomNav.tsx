...
export default function BottomNav() {
  ...
  return (
    <NavPortal><nav aria-label="Bottom navigation" className="fixed bottom-4 left-1/2 z-[1000] flex w-[calc(100%-2rem)] max-w-[430px] -translate-x-1/2 items-center gap-1 rounded-full border border-white/10 bg-zinc-900/90 p-2 text-zinc-400 shadow-2xl backdrop-blur-md backdrop-saturate-150 [padding-bottom:calc(0.5rem+env(safe-area-inset-bottom))]">
      {activeIdx >= 0 && (
        <span aria-hidden="true" className="pointer-events-none absolute inset-y-1 left-2 w-[calc((100%-1rem)/5)] rounded-full bg-white/10 shadow-[inset_0_1px_1px_rgba(255,255,255,0.12)] transition-transform duration-300 ease-out" style={{ transform:`translateX(calc(${activeIdx} * (100% + 0.25rem)))` }}/>
      )}
      {NAV.map(({ href, label, Icon, avatar }) => {
        const active = isActive(href, path)
        return (
          <button key={href} onClick={() => router.push(href)} aria-label={label} aria-current={active ? 'page' : undefined} className={`relative z-10 flex min-w-0 flex-1 flex-col items-center justify-center gap-1 rounded-full px-1 py-2 transition-all duration-300 ${active ? 'font-bold text-white' : 'font-medium text-zinc-400 hover:text-zinc-200'}`}>
